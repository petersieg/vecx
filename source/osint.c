...
int main(int argc, char *argv[]){
	const SDL_MessageBoxButtonData buttons[] = {
		/* .flags, .buttonid, .text */ 
		{ SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT, 0, "minestorm" },
		{ SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT, 1, "pinball" },
		{                                       0, 2, "pole" },
	...
	if (buttonid == -1) {
		...
		romfilename="rom.bin";
		sprintf(overfilename,"overlay.bmp");
	}
 	else {
		SDL_Log("Selection was %s", buttons[buttonid].text);
		sprintf(cartfilename,"rom/%s.bin",buttons[buttonid].text);
		sprintf(overfilename,"overlay/%s.bmp",buttons[buttonid].text);
